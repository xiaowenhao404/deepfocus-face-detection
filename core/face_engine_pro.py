...
class FaceEnginePro:
    ...
    def process_scene(
        ...
        # 检测人脸
        self.logger.info(f"检测人脸中（model={model}, upsample={upsample}）...")
        update_progress(20)
        ...
        if not boxes:
            return img, {"total": 0, "matches": 0, "best_dist": 1.0}
        ...
        for draw_idx, ((top, right, bottom, left), is_match, dist) in enumerate(results):
            ...
            if is_match:
                ...
                cv2.putText(
                    img, confidence_text, (text_x, text_y),
    ...
    def process_scene_with_stages(
        ...
        try:
            boxes = face_recognition.face_locations(
                rgb_img, number_of_times_to_upsample=upsample, model=model
            )
        except Exception as e:
            self.logger.warning(f"检测出错: {e}, 降级为 HOG")
            boxes = face_recognition.face_locations(
        ...
        if not boxes:
            ...
            return stages, {"total": 0, "matches": 0, "best_dist": 1.0}
        ...
        for draw_idx, ((top, right, bottom, left), is_match, dist) in enumerate(results):
            ...
            if is_match:
                ...
                cv2.putText(
                    final_img, confidence_text, (text_x, text_y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
                )
